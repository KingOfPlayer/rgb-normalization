...
class ModelFactory:
    @staticmethod
    def get_model(
        model_name: str, device: str, cache_dir: str = "/storage/embeding/", **kwargs
    ):
        base_cache_path = Path(cache_dir)
        factory_name, model_name = model_name.split("_", 1)
        if factory_name.startswith("OpenClip"):
            from capsules.EmbeddingExtraction.src.classes.sources.OpenClip import (
                OpenClipSoruce,
            )

            return OpenClipSoruce(
                model_name=model_name,
                device=device,
                cache_dir=base_cache_path / "OpenClip",
                **kwargs,
            )
        if factory_name.startswith("PerceptionEncoder"):
            ...
            from capsules.EmbeddingExtraction.src.classes.sources.PerceptionEncoder import (
                PerceptionEncoderSource,
            )

            return PerceptionEncoderSource(
                model_name=model_name,
                device=device,
                cache_dir=base_cache_path / "PerceptionEncoder",
                **kwargs,
            )
